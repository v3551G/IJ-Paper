...
\begin{document}
	...
	Given the dataset $\{(x_i,y_i)\}_{i=1}^n$, with input data $x_i \in \mathbb{R}^{p}$ and output data $y_i \in \mathbb{R}$ with class labels $y_i \in \{-1,1\}$, the following constrained optimization must be solved to obtain the LS-SVM representation of the hyperplane margin $||w||^{-1}$ maximization:
	...
	\end{bmatrix},
	\end{equation}
	where $y  = [y_1,...,y_n]^\mathrm{T} \in \mathbb{R}^n$, $1_v = [1,...,1]^\mathrm{T} \in \mathbb{R}^n$ and $\mathrm{I} \in \mathbb{R}^{n \times n}$ the identity matrix.
...
The centered kernel matrix $\tilde{K}$ is calculated as~\cite{scholkopf1998nonlinear}:
	\begin{align}
	\label{eq:centerKh}
	\tilde{K}_{ij} &= \Big(\phi(x_i) - \frac{1}{n}\sum_{m=1}^n\phi(x_m)\Big)^\mathrm{T} \Big(\phi(x_j) - \frac{1}{n}\sum_{k=1}^n\phi(x_k)\Big) \\
	...
	\begin{enumerate}
		...
		\end{equation}
		where $K_{i, i}$ denotes the diagonal element at index $i$ and $K_{., i}$ the $i$-th column of the kernel matrix. Ten iterations are usually enough to obtain a good approximation.	
		...
		\begin{enumerate}
			...
			\begin{equation}
			||\alpha^k|| = \frac{1}{n\lambda_k}, \mathrm{for \enskip all \enskip components \enskip}k = 1,... \,,h.
			\end{equation}
			\item Calculate the kernel matrix $K_t \in \mathbb{R}^{n \times h}$ for the given $h$-subset of observations in $n$ features.
			...
			\end{equation}
			with squared  Mahalanobis distances  $d^2 \in \mathbb{R}^{n}$, centered kernel matrix $ \tilde{K_t} \in \mathbb{R}^{n \times h}$, $\lambda^{-1} = [\lambda_1^{-1} ,...,\lambda_h^{-1}]^\mathrm{T} \in \mathbb{R}^{h}$ , $\alpha \in \mathbb{R}^{h \times h}$ and $^{\circ 2}$ the 
			element-wise square.
		...
		\begin{equation}
			...
			\end{cases}
		\end{equation}
		where $c$ is a threshold which we choose here as $c = \mathrm{max}(\chi_p^2(0.975), \mathrm{max}(d_h^2) )$, with $d_h^2 \in \mathbb{R}^h$ the distance vector of the observations in the $h$-subset. 
%		\begin{equation}
%		w_i =
%		\begin{cases}
...
%		\end{equation}
	\end{enumerate}
	After the kernel C-steps, you are left with an outlierfree dataset $\mathcal{J}_h \subset \mathcal{J}, |\mathcal{J}_h| = 2 \, h$, where $h$ is the number of regular observations in each class~\footnote{In the formulation the number of outliers is equal for each class. However the user can freely choose this number, this is especially useful in unbalanced classification problems.} and $\mathcal{J}$ the original trainingset.
...
%	\subsection{}
	...
	The second problem is solved by introducing a subset for each class \newline $y \in \{+1,-1\}$ called the "region of interest" (ROI), which consist of points close to the decision boundary. Determining the ROI is done separately for each class and consists out of two steps:
	\begin{enumerate}
	...
	\end{equation}
	where $1_v = [1;1;...;1]$ and a normalized kernel $\Omega \in \mathbb{R}^{n \times n}$ is assumed with respect to density estimation. Entropy quantifies the diversity or randomness of a system, which is essential to select landmark points such that no clumping is possible. In the fixed-size LS-SVM approach, one chooses a fixed working set of size $M$ which is initialized randomly. Afterwards, random points are swapped in and out. If the entropy increases, the swapped point is accepted, otherwise the original subset is kept. This process continues until the change in entropy is small or a fixed number of iterations is reached. 
...
%	\end{remark}
	...
	\begin{enumerate}
		...
		\item Determine a working set of size $l$ by randomly selecting support vectors out of the ROI.
...
Given the ROI of candidate support vectors for a class $y \in \{+1,-1\}$, the k-DPP selection algorithm is repeated for every class to select $2 \,l$ landmark points~\footnote{This corresponds to $l$ landmark points per class. In the formulation the number of points is chosen equal for each class. However the user is free to choose this number, which is particularity interesting for unbalanced datasets.}~\cite{kulesza2011k} : 
\begin{enumerate}
...
\hat{y} = \mathrm{sign}(K\beta  + b 1),
	\end{equation}
with $\hat{y} = [\hat{y}_1,...,\hat{y}_{2\,h}]^\mathrm{T}$ the prediction vector and $\beta = [\alpha_i y_i ,...,\alpha_i y_{2\,h}]^\mathrm{T}$, where $2\,h$ is the number of points in the outlierfree dataset ($h$ points per class).
...
\hat{y} = \mathrm{sign}(K_l\beta_l + K_{nl}\beta_{nl}  + b 1),
 ...
 The classier prediction equation finally boils down to:
	\begin{equation}
	\label{eq:LastClass}
\hat{y}(x) = \mathrm{sign}\Big(\sum_{i=1}^{2 \, l} K(x, x_i)\hat{\beta}_i  + b\Big).	
	\end{equation}
...
Given the outlierfree dataset from algorithm 1, the trained LS-SVM model based on this outlierfree dataset and the selected support vectors based on algorithm 3, the information transfer is done in the following way: 
	...
	\subsection{Summary RoS-LSSVM} 
	To finalize this section, we summarize the steps of the proposed algorithm. The following procedure is applied on a contaminated dataset $\mathcal{J} = \{(x_i,y_i)\}_{i=1}^n$, with input data $x_i \in \mathbb{R}^{p}$ and output data $y_i \in \mathbb{R}$ with class labels $y_i \in \{-1,1\}$:
	
\begin{enumerate}
	\item[]\textbf{Final algorithm: RoS-LSSVM}
	\item Start with the robust standardization of the dataset using equation~\eqref{eq:robustStand}.
	\item Run Algorithm 1 (Outlier detection using kernel c-steps) on each class. This results is an outlierfree dataset $\mathcal{J}_h \subset \mathcal{J}, |\mathcal{J}_h| = 2 \, h$, where $h$ is the number of regular observations in each class ($2 \, h$ in total for a 2-class problem).
	\item Train an LS-SVM classfier on this outlierfree dataset, which gives the support vector values and bias term. 
	\item Based on the support vector values, determine the ROI for each class using Algorithm 2.
	\item Select the final support vector values or landmark points for each class using a diverse sampling procedure (Algorithm 3 or 4). This results in the subset $\mathcal{J}_l \subset \mathcal{J}_h, |\mathcal{J}_l| = 2 \, l$, where $l$ is the number of landmark points in each class ($2 \, l$ in total for a 2-class problem).
	\item Transfer the information from the support vector values of not selected support vectors to the selected support vectors using algorithm 5. The result is the RoS-LSSVM classifier.
\end{enumerate}
