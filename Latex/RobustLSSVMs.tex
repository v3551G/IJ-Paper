...
\begin{equation}
	\hat{y}(x) = \mathrm{sign}\Big(\sum_{1}^{n} \alpha_i y_i K(x, x_j) + b\Big)	
...
\begin{equation}
	...
	\label{eq:prediction}
\end{equation}
$\hat{y}_i$ is the predicted output of the ith training data point. Note that $\alpha$ satisfies the linear constraint $\sum_{i=1}^{n} \alpha_i  e_i = 1$, and that the derivation for least squares support vector regression follows the same reasoning where the sign is omitted. %\cite{Choi2009}.
...
Finally, the centred kernel matrix of the $h$-subset $\tilde{K}_h$ is calculated as~\cite{scholkopf1998nonlinear}:
\begin{align}
\label{eq:centerKh}
...
\end{align}
where $ \tilde{K}^{(hh)} \in \mathbb{R}^{h \times h}$ and $1_h \in \mathbb{R}^{h}$ with all entries set to $1/h$. 
The centering of the kernel matrix of $n$ observations, given the $h$-subset follows a similar derivation:
\begin{align}
	\tilde{K}^{(h)} &= K^{(h)} - 1_h^T K^{(hh)} - K^{(h)}1_h + 1_h^T K^{(hh)}1_h,
\end{align}
with kernel matrix $ \tilde{K}^{(h)} \in \mathbb{R}^{n \times h}$ and the matrix $1_h \in \mathbb{R}^{h \times n}$ with all entries set to $1/h$.
