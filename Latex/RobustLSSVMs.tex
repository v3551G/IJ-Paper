...
\begin{figure}[!htb]
	...
	\caption{Figure \subref{fig:dmodel} illustreerd een normaal gedistribueerd classificatieprobleem (blauw, groen), waarbij beiden klassen disjuncte outliers bevatten uit eenzelfde materiaal groep, hier weergegeven in het rood. Na opsplitsen van dataset volgens hun class label [STAP2?] worden de $h_{init}$ dichtste punten bij de spatiale mediaan gebruikt als initiele subset voor het C-step algoritme [STAP3?]. Voor beiden classe is deze initiele subset in het groen, op figuur b, c respectively. Na iteratieve uitvoering van kernel C-steps [STAP4?], waarbij de $h$-subset grootte stelselmatig geexpanteerd wordt bekomen we uiteindelijk resultaat na reweighting [STAP5?], weergeven in figuren d en e. Uit de gedetecteerde reguliere observaties lijst worden vervolgens support vectors bepaald (zie sectie [REF])}
\end{figure}
