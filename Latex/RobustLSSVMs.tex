...
%	\end{enumerate} 
	...
	\end{align}
	with kernel matrix $ \tilde{K}_t \in \mathbb{R}^{t \times n}$ and the matrix $1_t \in \mathbb{R}^{t \times n}$ with all entries set to $1/n$. Having derived all required equations, the proposed pruning strategy starts with the standardization of each observation: 
