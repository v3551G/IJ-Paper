...
% Define block styles  
\tikzstyle{materia}=[draw, fill=orange!30, text width=6.0em, text centered,
minimum height=1em]
\tikzstyle{practica} = [materia, text width=11.5em, minimum width=10em,
minimum height=0.5em, rounded corners, drop shadow]
...
\newcommand{\background}[5]{%
	\begin{pgfonlayer}{background}
		...
		% Draw the background
		\path[fill=gray!10,rounded corners, draw=black!80]
		(a1) rectangle (a2);
		\path (a1.east |- a1.south)+(1.1,-0.4) node (u1)[texto]
		{\scriptsize{#5}};
\end{pgfonlayer}}
...
\begin{document}
	...
	\begin{tikzpicture}[scale=0.9,transform shape]	
		\path \practica {1}{Robust standarization};
		
		\path (p1.south)+(-2.5,-1.8) \practica{2}{Kernel Spatial Median};
		\path (p2.south)+(0.0,-1.0) \practica{3}{Kernel C-steps};
		
		\path (p1.south)+(+2.5,-1.8) \practica{4}{Kernel Spatial Median};
		\path (p4.south)+(0.0,-1.0) \practica{5}{Kernel C-steps};
		\background{p2}{p2}{p5}{p5}{Outlier-detection}
		
		\path [line] (p1.south) -- +(0.0,-0.5) -- +(-2.5,-0.5)
		...
		\path (p5.south)+(-2.5,-1.5) \practica{6}{Train LS-SVM classifier};

		\path [line] (p5.south) -- +(0.0,-0.5) -- +(-2.5,-0.5)
		-- node [above, midway] {} (p6);
		\path [line] (p3.south) -- +(0.0,-0.5) -- +(+2.5,-0.5)
		-- node [above, midway] {} (p6);
						
		\path (p6.south)+(-2.5,-1.8) \practica{7}{Determine ROI};		
		\path (p7.south)+(0,-1.0) \practica{8}{Sample ROI: DPP/Entropy};	

		\path (p6.south)+(+2.5,-1.8) \practica{9}{Determine ROI};
		\path (p9.south)+(0.0,-1.0) \practica{10}{Sample ROI: DPP/Entropy};

		\path [line] (p6.south) -- +(0.0,-0.5) -- +(-2.5,-0.5)
		-- node [above, midway] {} (p7);
		\path [line] (p6.south) -- +(0.0,-0.5) -- +(+2.5,-0.5)
		-- node [above, midway] {} (p9);

		\background{p7}{p7}{p10}{p10}{SV-Sampling}		
		\path (p10.south)+(-2.5,-1.8) \practica{11}{Transfer information};

		\path [line] (p10.south) -- +(0.0,-0.5) -- +(-2.5,-0.5)
		-- node [above, midway] {} (p11);
		\path [line] (p8.south) -- +(0.0,-0.5) -- +(+2.5,-0.5)
		-- node [above, midway] {} (p11);

		\background{p11}{p11}{p11}{p11}{Model-update}
	\end{tikzpicture}
