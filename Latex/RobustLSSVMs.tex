...
Note that $\alpha$ satisfies the linear constraint $\sum_{i=1}^{n} \alpha_i  e_i = 1$, and that the derivation for least squares support vector regression follows the same reasoning where the sign is omitted in the prediction function. %\cite{Choi2009}.
	...
	\label{sec:proposed}
	We first start by laying out the main methodology of our proposed method, where our novel pre-and post-processing operations are shown in grey on figure [REF]. A given input dataset is first standardized, followed by our non-linear outlier detection routine,(section [???]) in order to extract a list of regular observations. These candidate support vectors are then used to train a standard, weighted LS-SVM classifier. Because the solution lacks sparsity - apart from the removal of outliers - a second algorithm is used (section [REF??]) to extract the support vectors based on entropy - or negative correlation (section [REF??]). Finally, the information of the detected regular observations is transferred to the support vectors to improve its classification efficiency, as highlighted in the simulations.  
